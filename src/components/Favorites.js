...
const Favorites = ({ history }) => {
      ...
      try {
        return await Promise.all(
          favorites.map(async x => {
            ...
            const [{ WeatherText, Temperature, WeatherIcon }] = await response.json();
            return {
              WeatherText,
              Temperature,
              WeatherIcon,
  ...
  const removeFavorite = (e, favorite) => {
    ...
    const i = favorites.findIndex(x => JSON.stringify(x) === JSON.stringify(favorite));
    temp.splice(i, 1);
    setFavorites([...temp]);
  };
          ...
          {favorites.map(x => (
            <Item key={x.Key} onClick={() => selectFavorite(x)}>
              ...
              <Content>
                ...
                <Temperature>
                  {Math.floor(metric ? x.Temperature.Metric.Value : x.Temperature.Imperial.Value)}
                  &deg;
                </Temperature>
